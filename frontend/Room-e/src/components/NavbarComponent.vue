...
<template>
  <header class="text-gray-400 bg-gray-800 body-font">
    <div class="container mx-auto flex flex-wrap p-5 flex-col md:flex-row items-center">
      <router-link :to="{ path: '/' }" @click.prevent="$route.path === '/' ? window.location.reload() : null" class="flex title-font font-medium items-center text-white mb-4 md:mb-0">
      ...
      <nav class="md:ml-auto flex flex-wrap items-center text-base justify-center">
        <router-link to="/" v-if="isLogged && isAdmin" class="mr-5 hover:pink-500 hover:cursor-pointer">View All Rooms</router-link>
        <router-link to="/" v-if="!isAdmin" class="mr-5 hover:text-pink-500 hover:cursor-pointer">View All Available Rooms</router-link>
        <router-link to="/" v-if="isLogged && isAdmin" class="mr-5 hover:text-pink-500 hover:cursor-pointer">View All Reservations</router-link>
